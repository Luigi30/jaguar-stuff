...
{
  jag_dsp_wait();
  memcpy(&dsp_matrix_vector, &rotation, sizeof(Vector3FX));
  dsp_matrix_ptr_result = result;
  
  DSP_START(dsp_matrix_x_rotation);
}

void Matrix44_Y_Rotation(Vector3FX rotation, Matrix44 *result)
{
  MMIO32(0x60010) = (long)result;

  jag_dsp_wait();
  memcpy(&dsp_matrix_vector, &rotation, sizeof(Vector3FX));
  dsp_matrix_ptr_result = result;
  
  DSP_START(dsp_matrix_y_rotation);
}
...
void Matrix44_Z_Rotation(Vector3FX rotation, Matrix44 *result)
{
  ...
  DSP_START(dsp_matrix_z_rotation);
}	
