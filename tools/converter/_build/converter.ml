...
let tool_info () =
  "; Converted with 'Jaguar image converter' (version x.xx) by Seb/The Removers\n"
...
let reset_ascii,finish_ascii,output_ascii_long,output_ascii_word,output_ascii_byte =
  ...
  and output_ascii header stream value =
    ...
    incr counter;
    if !counter >= counter_by_line then begin
...
let info_string =
  let prelude = "Jaguar image converter by Seb/The Removers (version x.xx)" in
